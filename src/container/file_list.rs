...
pub fn FileList() -> impl IntoView {
    ...
    let handle_click = move |file: &FileInfo, index: usize, navigate: &dyn Fn(&str, NavigateOptions)| {
        if file.is_dir {
            let new_path = format!("{}/{}", path.read(), file.name);
            set_path.set(new_path.clone());
            set_url_params(&HashMap::from([("path".to_string(), new_path)]));
        } else {
            navigate(&format!("/reader?page={}", index), Default::default());
        }
    };
                        ...
                        .map(|(index, file)| {
                          ...
                          view! {
                            <ListItem on_click=move |_| {
                              handle_click(&file_clone, index, &navigate);
                            }>
                                <div>{file.name}</div>
                            </ListItem>
                          }
